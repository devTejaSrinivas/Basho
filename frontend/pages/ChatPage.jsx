...
const handleFileUpload = (e) => {
  ...
  if (files.length + capturedImages.length + uploadedImages.length > 5) {
    alert(`You can only add up to 5 images in total. You already have ${capturedImages.length + uploadedImages.length} images.`);
    return;
  }
    ...
    >
          ...
          >
            ...
            {message.imageUrls && message.imageUrls.length > 0 ? (
              <div style={{ 
                ...
                maxWidth: "70%",
                background: message.sender === "user" ? "rgb(0, 120, 215)" : "#333",
                borderRadius: "8px",
                overflow: "hidden"
              }}>
                {/* Message text (if any) */}
                ...
                <div style={{ 
                  display: "grid", 
                  gridTemplateColumns: message.imageUrls.length > 1 ? "repeat(2, 1fr)" : "1fr",
                  gap: "2px", 
                  padding: "2px"
                }}>
      ...
      >
        <input
          ...
          placeholder={
            pendingMedia.length > 0 ? "Add a caption..." : "Type your message..."
          }
          style={{
            flex: 1,
            padding: "0.5rem",
        ...
        <button
          type="submit"
          disabled={isLoading || (pendingMedia.length === 0 && currentMessage.trim() === '')}
          style={{
            padding: "0.5rem 1rem",
            background: isLoading || (pendingMedia.length === 0 && currentMessage.trim() === '') 
              ? "#555" 
              : "#007bff",
            color: "#fff",
            border: "none",
            borderRadius: "4px",
            cursor: isLoading || (pendingMedia.length === 0 && currentMessage.trim() === '') 
              ? "not-allowed" 
              : "pointer",
            display: "flex",
            alignItems: "center",
        ...
        >
          {isLoading ? (
            <span style={{ display: "inline-block", width: "16px", height: "16px", borderRadius: "50%", 
